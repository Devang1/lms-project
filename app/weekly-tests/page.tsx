...
export default async function WeeklyTestsPage({
  ...
  const where: Prisma.TestWhereInput = !userId
    ...
    : role === "STUDENT"
      ? { course: { enrollments: { some: { userId } } }, status: { in: ["SCHEDULED", "ACTIVE"] } }
      : role === "TEACHER"
  ...
  const selectedTest = tests.find((test) => test.id === params.testId);
  const selectedResult = selectedTest && userId
    ? selectedTest.results
      .filter((result) => result.userId === userId)
      .sort((left, right) => right.submittedAt.getTime() - left.submittedAt.getTime())[0]
    : null;
  const existingEvents = selectedTest && session?.user
    ? await prisma.suspiciousActivity.findMany({
      where: { userId: session.user.id, testId: selectedTest.id },
      orderBy: { createdAt: "asc" },
      select: { event: true, createdAt: true }
    })
    : [];
  const selectedAnalysis = selectedTest && selectedResult && session?.user
    ? buildResultAnalysis({
      questions: selectedTest.questions,
      result: selectedResult,
      studentId: session.user.id,
      testId: selectedTest.id,
      events: existingEvents.map((event) => ({ event: event.event, createdAt: event.createdAt }))
    })
    : null;
    ...
    <AppShell role={role}>
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 sm:mb-8">
          <p className="text-md font-medium text-primary/80 sm:text-sm">Anti-cheat interactive testing</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight sm:text-3xl lg:text-4xl">
            Secure assessment arena
          </h1>
          {role === "STUDENT" ? (
            <p className="mt-2 text-xs text-muted-foreground sm:text-sm">
              Showing tests only from courses you are enrolled in.
            </p>
          ) : null}
        </div>):""}
        ...
        {selectedTest && selectedAnalysis ? (
          <div className="mb-6 space-y-4 sm:mb-8 sm:space-y-6">
            <Button asChild variant="outline" size="sm" className="w-fit gap-2 sm:size-default">
              <Link href="/weekly-tests">
                <ArrowLeft className="h-3 w-3 sm:h-4 sm:w-4" />
                <span className="text-xs sm:text-sm">Back to tests</span>
              </Link>
            </Button>
            <ResultAnalysisCard title={selectedTest.title} analysis={selectedAnalysis} />
          </div>
          ...
          <div className="mb-6 space-y-4 sm:mb-8 sm:space-y-6">
            ...
            <ExamClient
              ...
              durationMin={selectedTest.durationMin}
              questions={personalizeQuestions(selectedTest.questions, session.user.id, selectedTest.id)}
              existingEvents={existingEvents.map((event) => ({ event: event.event, createdAt: event.createdAt.toISOString() }))}
            />
          </div>
        ) : tests.length ? (
          <div className="mb-6 rounded-lg border bg-card p-3 text-xs text-muted-foreground sm:mb-8 sm:p-4 sm:text-sm">
            Select a test below and press <span className="font-medium text-foreground">Start test</span> to open the secure exam screen.
          </div>
        ) : (
        ...
        <div className="grid gap-3 sm:gap-4 lg:grid-cols-2">
          {tests.map((test) => {
            const hasSubmitted = test.results.some((result) => result.userId === userId);
            return (
              <Card 
                key={test.id} 
                className={`transition-all duration-200 hover:shadow-md ${
                  test.id === selectedTest?.id ? "border-2 border-primary/50" : ""
                }`}
              >
                ...
                <CardContent className="space-y-3 p-4 pt-0 sm:space-y-4 sm:p-6 sm:pt-0">
                  ...
                  <div className="space-y-2 text-xs text-muted-foreground sm:space-y-2.5 sm:text-sm">
                    ...
                    <p className="flex items-center gap-2">
                      ...
                      <span>
                        {test.questions.length} personalized questions - {test.results.length} submission{test.results.length !== 1 ? "s" : ""}
                      </span>
                    </p>
                  </div>
                  ...
                  <Button 
                    asChild 
                    size={hasSubmitted ? "default" : "default"}
          ...
          {!tests.length && (
            <Card>
              <CardContent className="p-6 text-center text-sm text-muted-foreground sm:p-8">
                <p className="text-xs sm:text-sm">No course tests are available yet.</p>
