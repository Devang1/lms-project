      ...
      <div className="mx-auto max-w-2xl pb-20 lg:pb-0">
        <div className="sticky top-0 z-30 -mx-4 border-b bg-background/95 px-4 py-3 backdrop-blur sm:mx-0 sm:rounded-b-md sm:border sm:border-t-0">
          <div className="flex items-center justify-between gap-3">
            <Button asChild variant="ghost" size="icon" aria-label="Back to feed">
              <Link href="/feed"><ArrowLeft size={20} /></Link>
            </Button>
            <div className="min-w-0 text-center">
              <p className="font-semibold leading-5">Create post</p>
              <p className="text-xs text-muted-foreground">Share a study update</p>
            </div>
        ...
        <Card className="mt-4 overflow-hidden">
          <CardContent className="p-0">
            <FeedComposer userName={session?.user.name ?? "Student"} />
          </CardContent>
        </Card>
      </div>
    </AppShell>
